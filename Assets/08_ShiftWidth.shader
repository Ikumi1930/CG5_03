    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag(v2f i) : SV_Target 
            {
                ...
                for(float py = -_Sigma*2; py <= _Sigma*2; py += _StepWidth) // xyでσの幅で色を取得
                {
                    for(float px = -_Sigma*2; px <= _Sigma*2; px += _StepWidth) 
                    {
						float2 pickUV = i.uv + float2(px, py);
						fixed weight = Gaussian(i.uv, pickUV, _Sigma);
						col += tex2D(_MainTex, pickUV) * weight;//Gaussianで取得した重みを色にかける
						totalWeight += weight;//かけた重みの合計値を控えておく
                    }
                }
